...
const handleSubmit = async () => {
  try {
   ...
   setLoading(true);
    ...
    form.append(
      "specialistsCount",
      formData.specialistsCount
     
    );

    form.append(
      "doctorsCount",
      formData.doctorsCount
    );

    form.append(
      "visitMode",
      formData.visitMode.toLowerCase()
    );
...
{step !== 1 && (
  ...
  {["1 Doctor", "2 Doctors", "3 Doctors", "More than three Doctors"].map((item) => (
    <label
      key={item}
  ...
  {["Online", "Offline"].map((item) => (
    ...
    >
      ...
      {item}
        ...
        {/* Address */}
        <div className="flex flex-col md:flex-row items-start md:items-center gap-3 mb-5">
          <label className="w-full md:w-48 font-semibold">Clinic Address</label>
          <input name="address" 
          type="text"
          onChange={handleChange} 
          className="w-full bg-gray-200 p-2 rounded-md outline-none"/>
       ...
       {/* City + State */}
        ...
        {/* Contact */}
        <div className="flex flex-col md:flex-row gap-3 mb-5">
          <label className="w-full md:w-48 font-semibold">Contact Number</label>
          <input name="contact"
          type="number"
           onChange={handleChange} 
           className="w-full md:w-1/2 bg-gray-200 p-2 rounded-md outline-none"/>
