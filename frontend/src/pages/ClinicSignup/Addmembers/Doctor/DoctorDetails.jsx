  ...
  return alert("Please verify OTP first");
    ...
    try {
      ...
      if (formData.panDocument) {
        form.append("panDocument", formData.panDocument);
      }
      ...
      if (formData.qualificationCertificate) {
        form.append("qualificationCertificate", formData.qualificationCertificate);
      }

      const response = await api.post("/clinic/doctor/register", form, {
        headers: {
    ...
    <form 
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[#1A5F48] via-[#89C9CA] to-[#C6EBE8] p-4">
        <div className="bg-white w-full max-w-4xl rounded-xl shadow-lg p-6 md:p-8 border">
          ...
          <div className="flex items-center md:gap-2 mb-5">
            <label className="text-sm sm:text-lg font-semibold w-28 sm:w-48 ml-2 sm:ml-10">
              Specialization <span className="px-2 text-red-600">*</span>
            </label>
            <input
              required
              name="specialization"
...
export default Clinic_doc_Details;